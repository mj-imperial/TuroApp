...
@Composable
fun ScreeningExamListScreen(
    navController: NavController,
    sessionManager: SessionManager,
    viewModel: ScreeningExamListViewModel
){
    ...
    val exams = uiState.exams.sortedBy { it.screeningName }
        ...
        content = {
            if (uiState.loading){
                ...
                PullToRefreshBox(
                    ...
                    onRefresh = { viewModel.getScreeningExams() },
                ) {
                    LazyColumn(
                        modifier = Modifier
                            .fillMaxSize()
                        ...
                        items(exams) { exam ->
                            ScreeningExamListCard(
                                screeningExamName = exam.screeningName,
                                onNavigateScreeningQuiz = {
                                    navController.navigate(Screen.ScreeningExamDetail.createRoute(exam.screeningId))
                                }
                            )
...
@Composable
fun ScreeningExamListCard(
    screeningExamName: String,
    onNavigateScreeningQuiz: () -> Unit
){
    Card(
        modifier = Modifier
            .padding(20.dp)
            .fillMaxWidth()
            .clickable(onClick = onNavigateScreeningQuiz),
